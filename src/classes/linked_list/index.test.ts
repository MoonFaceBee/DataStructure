...
Deno.test("Includes", () => {
  ...
  assertEquals(myLinkedList.includes(2), true);
  assertEquals(myLinkedList.includes(4), false);
});

Deno.test("Proxy Indexer", () => {
  const myLinkedList = new LinkedList<number>();
  ...
  assertEquals(myLinkedList[1], 2);

  myLinkedList[1] = 10;

  assertEquals(myLinkedList[1], 10);
});
